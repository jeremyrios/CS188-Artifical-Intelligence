...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        "*** YOUR CODE HERE ***"
        #eval func1:  1 / (#foodLLeft + 0.5*closestFood) if closestGhost > 2 else 0
        foodLeft = newFood.asList()
        numFoodLeft = len(foodLeft)
        #return gameScore when no food left
        if numFoodLeft == 0:
          return successorGameState.getScore()

        closestFood = min([manhattanDistance(newPos, food) for food in foodLeft])
        newGhostPos = [ghostState.getPosition() for ghostState in newGhostStates]
        closestGhost = min([manhattanDistance(newPos, ghost) for ghost in newGhostPos])
        result = 1 / (numFoodLeft + 0.05*closestFood) if closestGhost > 2.0 else 0
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def minimizer(self, gameState, depth, numGhosts):
        ...
        result = (None, float(sum(newScores) / float(len(newScores))))
        return result
...
def betterEvaluationFunction(currentGameState):
    ...
    if closestGhost > 2 or min(newScaredTimes) != 0 :
      result = 1 / ((numFoodLeft + 0.05*closestFood) * (numFoodLeft + 0.05 * closestFood))
    else:
      result = 0

    return result
    """
    finalScore = 0.0 + currentGameState.getScore()
