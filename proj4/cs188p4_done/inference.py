...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        # P(x1 | t...) = SUM_allE_x0's(P(x1 | x0, t+1) * P(x0 | t,..))
        allPossible = util.Counter()
        if gameState.isWin():
          allPossible[self.getJailPosition()] = 1.0
        else:
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        rand = int(random.random()*numLegalPositions)

        self.particlesList = []
        for i in range(self.numParticles):
          self.particlesList.append(self.legalPositions[(i+rand) % numLegalPositions])
    ...
    def elapseTime(self, gameState):
        ...
        if gameState.isWin():
          newParticlesList = [self.getJailPosition()] * self.numParticles
        else:
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.legalPositions + [self.getJailPosition()]:
          beliefs[p] = self.particlesList.count(p)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particlesList = []
        for i in range(self.numParticles):
          particle = self.legalPositions[(i+rand) % numLegalPositions]
          self.particlesList.append((particle,) * self.numGhosts)
    ...
    def replaceOnePositionInParticle(self, particle, sub, index):
