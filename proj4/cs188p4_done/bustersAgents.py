...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ## [(maxArg_b1, maxValue_b1), (maxArg_b2, maxValue_b2), ...]
        #maxPairsList = [(beliefs.argMax(), max(beliefs.values())) for beliefs
        #                in livingGhostPositionDistributions
        #                if max(beliefs.values()) != 1]
        
        # get the most likely ghost position
        maxKeysList = []
        maxValuesList = []
        for beliefs in livingGhostPositionDistributions:
          argMax = beliefs.argMax()
          maxKeysList.append(argMax)
          maxValuesList.append(beliefs[argMax])

        mostLikelyGhostPosition = maxKeysList[maxValuesList.index(max(maxValuesList))]
        ...
        for act in legal:
          ...
          mazeDistList.append(self.distancer.getDistance(successorPosition,
                                                mostLikelyGhostPosition))
        
        action = legal[mazeDistList.index(min(mazeDistList))]
